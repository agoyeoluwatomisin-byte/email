...
export async function sendOutboundEmail({ userId, threadId, to, cc, bcc, replyTo, subject, text, html, attachments = [], inReplyTo, references }) {
  ...
  if (!recipients.length || recipients.some((address) => !isEmail(address))) throw new Error('Every recipient must be a valid email address.');
  ...
  for (const attachment of Array.isArray(attachments) ? attachments.slice(0, 10) : []) {
    const content = attachment.content ? Buffer.from(attachment.content, 'base64') : await loadAttachmentContent(attachment);
    if (!content) throw new Error(`Attachment ${attachment.filename || attachment.name || 'file'} is unavailable.`);
    resendAttachments.push({ filename: attachment.filename || attachment.name || 'attachment', content, contentType: attachment.content_type || attachment.contentType || 'application/octet-stream' });
  ...
  const result = await resend.emails.send({
    ...
    subject: String(subject || '').trim().slice(0, 998),
    text: String(text || '').trim().slice(0, 100000),
  ...
  const { error: threadError } = await supabaseAdmin.from('threads').upsert({ thread_id: actualThreadId }, { onConflict: 'thread_id', ignoreDuplicates: true });
  if (threadError) throw threadError;
  const { data: stored, error: storeError } = await supabaseAdmin.from('emails').insert({
    ...
    subject: String(subject || '').trim().slice(0, 998),
    text_body: String(text || '').trim().slice(0, 100000),

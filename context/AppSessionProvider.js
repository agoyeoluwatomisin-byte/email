...
const AppSessionContext = createContext({ user: null, role: null, displayName: '', isAuthenticated: false, isReady: false, refresh: async () => {} });
...
export function AppSessionProvider({ children }) {
  ...
  const load = async () => {
    try {
      ...
      setSession({ user: data.user || null, role: data.user?.role || null, displayName: data.user?.displayName || data.user?.display_name || '', isAuthenticated: Boolean(data.authenticated) });
    } catch (error) {

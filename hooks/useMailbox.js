...
export default function useMailbox({
  ...
  const load = useCallback(
    async (override = {}) => {
      ...
      if (mode === 'inbox') {
        ...
        if (threadResponse.ok) {
          ...
          loadedEmails.forEach((email) => {
            const thread = state.get(email.thread_id);
            if (thread) {
              email.claimed_by = thread.claimedBy;
              email.status = thread.status;
            }
          });
        }
      }
      ...
      setUnreadThreadIds([
        ...new Set(
          loadedEmails.filter((email) => email.direction === 'inbound' && !email.read).map((email) => email.thread_id),
        ),
      ]);
      setLoading(false);
    },
  ...
  useEffect(() => {
    ...
    Promise.all([fetch(`/api/threads/${selectedThread}/activity`), fetch(`/api/threads/${selectedThread}/notes`)])
      .then(async ([activityResponse, notesResponse]) => {
  ...
  const threads = useMemo(() => {
    ...
    for (const email of emails) {
      if (!grouped.has(email.thread_id)) grouped.set(email.thread_id, []);
      grouped.get(email.thread_id).push(email);
    }
    const values = [...grouped.values()].map((thread) =>
    ...
    return folderFiltered.filter(
      (thread) =>
        selectedMailbox === 'all' ||
        thread.some((message) => (message.mailboxAddresses || []).includes(selectedMailbox)),
    );
  ...
  const mailboxGroups = useMemo(() => {
    ...
    emails.forEach((email) =>
      (email.mailboxAddresses || []).forEach((address) => counts.set(address, (counts.get(address) || 0) + 1)),
    );
    return [...counts]
      .map(([value, count]) => ({ value, label: value.split('@')[0], count, color: '#1769aa' }))
    ...
    async (threadId) => {
      ...
      if (mode === 'inbox') {
        ...
        if (currentUser) {
          ...
          if (!response.ok) {
            const data = await response.json().catch(() => ({}));
            setClaimError(data.error || 'This thread is already claimed.');
          }
        }
      }
    },
  ...
  const bulkAction = useCallback(
    async (action, ids = selectedThreadIds) => {
  ...
  const updateStatus = useCallback(
    async (status) => {
      ...
      const response = await fetch('/api/threads', {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ threadId: selectedThread, status }),
      });
      if (response.ok) await load();
    },
    [load, selectedThread],
  ...
  const addNote = useCallback(
    async (body) => {
      ...
      const response = await fetch(`/api/threads/${selectedThread}/notes`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
      ...
      if (response.ok) {
        ...
        setThreadNotes((current) => [...current, data.note]);
      }
    },
    [selectedThread],
  );
...
function parseMailboxAddresses(value) {
  ...
  return values
    .map((item) => (String(item).match(/<([^>]+)>/)?.[1] || String(item)).trim().toLowerCase())

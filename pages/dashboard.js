...
export default function Dashboard() {
    ...
    const load = async () => {
      ...
      try {
        ...
        const responseRate = inbound.length
          ? Math.round(
              (outbound.filter((email) => inbound.some((message) => message.from_address === email.to_address)).length /
                inbound.length) *
                100,
            )
          : 0;
  ...
  const cards = useMemo(
    () => [
      ...
      {
        label: 'CSAT average',
        value: analytics?.csatAverage ? analytics.csatAverage.toFixed(1) : '—',
        accent: '#f59e0b',
      },
    ],
    [stats],
              ...
              {statusColumns.map((column) => (
                <div key={column.value} className="boardColumn" style={styles.boardColumn}>
                  ...
                  <div style={styles.boardItems}>
                    {threads
                      .filter((thread) => thread.status === column.value)
                      .map((thread) => (
                        <div key={thread.threadId} style={styles.threadCard}>
                          ...
                          <div style={styles.threadCardMeta}>
                            {thread.unread ? 'Unread' : 'Read'} · {thread.messageCount} message
                            {thread.messageCount === 1 ? '' : 's'}
                          </div>
                          <div style={styles.statusActions}>
                            {statusColumns
                              .filter((option) => option.value !== thread.status)
                              .map((option) => (
                                <button
                                  key={option.value}
                                  type="button"
                                  style={styles.statusButton}
                                  onClick={() => updateThreadStatus(thread.threadId, option.value)}
                                >
            ...
            <section className="dashboard-two-column" style={styles.twoColumn}>
              ...
              <div style={styles.card}>
                ...
                {(analytics.agents || []).length === 0 ? (
                  <p style={styles.empty}>No replies in this range.</p>
                ) : (
                  analytics.agents.map((agent) => (
                    <div key={agent.email} style={styles.tagRow}>
                      <span>{agent.email}</span>
                      <strong>
                        {agent.replies} replies · {agent.resolved} resolved
                      </strong>
                    </div>
                  ))
                )}
                <p style={styles.chartMeta}>
                  Median resolution: {analytics.medianResolutionMinutes ?? '—'} min · Avg first response:{' '}
              ...
              <div style={styles.trendList}>
                {stats.dailyTrend.map((day) => {
                  const maxValue = Math.max(
                    ...stats.dailyTrend.flatMap((item) => [item.inboundCount, item.outboundCount]),
                    1,
                  );
                  return (
                    <div key={day.label} style={styles.trendItem}>
                      ...
                      <div style={styles.trendBars}>
                        <span
                          style={{
                            ...styles.trendBar,
                            height: `${(day.inboundCount / maxValue) * 100}%`,
                            background: '#60a5fa',
                          }}
                        />
                        <span
                          style={{
                            ...styles.trendBar,
                            height: `${(day.outboundCount / maxValue) * 100}%`,
                            background: '#34d399',
                          }}
                        />
                      </div>
...
function VolumeChart({ volume }) {
  const values = Object.entries(volume)
    .sort(([left], [right]) => left.localeCompare(right))
    .slice(-30);
  const maximum = Math.max(...values.map(([, value]) => value), 1);
  const points = values
    .map(([, value], index) => `${(index / Math.max(values.length - 1, 1)) * 100},${100 - (value / maximum) * 90}`)
    .join(' ');
  return (
    <svg viewBox="0 0 100 110" role="img" aria-label="Message volume chart" style={styles.chart}>
      <polyline points={points} fill="none" stroke="#60a5fa" strokeWidth="2" vectorEffect="non-scaling-stroke" />
      {values.map(([day, value], index) => (
        <text
          key={day}
      ...
      <text x="2" y="8" fontSize="4" fill="#9fb2c6">
        {maximum}
      </text>
    </svg>
  );
      ...
      {entries.map(([label, value]) => (
        <div key={label} style={styles.barRow}>
          ...
          <span style={styles.barTrack}>
            <span style={{ ...styles.barFill, width: `${(value / maximum) * 100}%`, background: color }} />
          </span>
          <strong>{value}</strong>
        </div>
      ))}
...
const styles = {
  ...
  dateFilters: { display: 'flex', gap: 8, color: 'var(--app-muted)', fontSize: 12 },
  kicker: {
    margin: 0,
  ...
  title: { margin: '4px 0 0', fontSize: 32, color: 'var(--app-text)' },
  ...
  boardColumn: { minWidth: 220, background: 'var(--app-surface)', borderRadius: 12, padding: 12, border: '1px solid var(--app-border)' },
  boardHeading: {
    display: 'flex',
  ...
  threadCard: { background: 'var(--app-surface-raised)', border: '1px solid var(--app-border)', borderRadius: 10, padding: 10 },
  threadCardSubject: { color: 'var(--app-text)', fontWeight: 700, fontSize: 13, overflowWrap: 'anywhere' },
  threadCardContact: { color: 'var(--app-muted)', fontSize: 11, marginTop: 5, overflowWrap: 'anywhere' },
  threadCardMeta: { color: 'var(--app-muted)', fontSize: 10, marginTop: 6 },
  ...
  cardLabel: { color: 'var(--app-muted)', fontSize: 12, textTransform: 'uppercase', letterSpacing: 0.7 },
  cardValue: { marginTop: 12, fontSize: 32, fontWeight: 700, color: 'var(--app-text)' },
  sectionTitle: { margin: '0 0 16px', color: 'var(--app-text)', fontSize: 20 },
  ...
  barTrack: { height: 10, borderRadius: 999, background: 'var(--app-surface-raised)', overflow: 'hidden' },
  barFill: { display: 'block', height: '100%', borderRadius: 999 },
  chartMeta: { color: 'var(--app-muted)', fontSize: 12, marginBottom: 0 },
  loadingWrap: { display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: 16 },
  loadingCard: {
    height: 110,
    borderRadius: 16,
    background: 'linear-gradient(90deg, var(--app-surface-raised) 25%, var(--app-border) 50%, var(--app-surface-raised) 75%)',
    backgroundSize: '200% 100%',
    animation: 'pulse 1.2s ease-in-out infinite',
  },
  empty: { color: 'var(--app-muted)', margin: 0 },
  ...
  trendLabel: { color: 'var(--app-muted)', fontSize: 11 },
  ...
  rowTitle: { fontWeight: 600, color: 'var(--app-text)' },
  rowMeta: { color: 'var(--app-muted)', fontSize: 12, marginTop: 4 },
  rowDate: { color: 'var(--app-muted)', fontSize: 12, whiteSpace: 'nowrap' },
};

  ...
  return (
    <main style={styles.main}>
      <div style={styles.header}>
        <h1>Contacts</h1>
        <a href="/api/export?format=contacts">Export CSV</a>
      </div>
      <input
        style={styles.search}
        value={search}
      ...
      {contacts.map((contact) => (
        <article key={contact.id} style={styles.row}>
          <div>
          ...
          {contact.vip && <span style={styles.vip}>VIP</span>}
        </article>
      ))}
    </main>
  );
...
const styles = {
  ...
  vip: { color: '#fbbf24', fontWeight: 700 },
};

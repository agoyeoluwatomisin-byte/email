...
export default function Outbox() {
  ...
  const mailbox = useMailbox({
    ...
    onSelectedThreadChange: (threadId) =>
      router.push(threadId ? { pathname: '/outbox', query: { thread: threadId } } : '/outbox', undefined, {
        shallow: true,
      }),
  });
  ...
  const rail = (
    <FolderRail
      ...
      onMailbox={mailbox.setSelectedMailbox}
    ...
    <section className="mail-list-pane">
      <header className="mail-list-header">
        ...
        <div className="mail-filter-tools">
          <span className="mail-count">{mailbox.threads.length} sent conversations</span>
  ...
  const reading = mailbox.activeThread ? (
    <section className="mail-reading-pane">
      <ThreadHeader
        ...
        label="sent"
        status={mailbox.activeThread[0].deliveryStatus || 'sent'}
        claimedBy={null}
        activityCount={0}
    ...
    <section className="mail-reading-pane">
      <EmptyState
        title="Select a sent conversation"
        message="Choose a message from the list to inspect it."
        icon="send"
      />
    </section>
  );

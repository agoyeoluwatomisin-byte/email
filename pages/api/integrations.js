...
export default async function handler(req, res) {
  ...
  if (req.method === 'GET') {
    const { data } = await supabaseAdmin
      ...
      .select('webhook_url, browser_notifications')
  ...
  if (req.method === 'PUT') {
    const values = {
      ...
      webhook_url: String(req.body?.webhookUrl || '').slice(0, 1000) || null,
      webhook_secret: String(req.body?.webhookSecret || '').slice(0, 200) || null,
      browser_notifications: Boolean(req.body?.browserNotifications),
      updated_at: new Date().toISOString(),
    };
    if (values.webhook_url && !/^https:\/\//i.test(values.webhook_url))

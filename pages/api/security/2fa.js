...
export default async function handler(req, res) {
  ...
  if (req.method === 'POST') {
    ...
    const { error } = await supabaseAdmin.from('users').update({ totp_secret: secret, totp_enabled: false, updated_at: new Date().toISOString() }).eq('id', session.user.id);
    if (error) return res.status(500).json({ error: 'Failed to start 2FA setup.' });
    return res.status(200).json({ secret, otpauth: `otpauth://totp/Agosoft:${encodeURIComponent(session.user.email)}?secret=${secret}&issuer=Agosoft` });
  }
  if (req.method === 'PUT') {
    const { data: user } = await supabaseAdmin.from('users').select('totp_secret').eq('id', session.user.id).single();
    if (!verifyTotp(user?.totp_secret, String(req.body?.code || ''))) return res.status(400).json({ error: 'Invalid authenticator code.' });
    const recoveryCodes = Array.from({ length: 8 }, () => crypto.randomBytes(5).toString('hex'));
    const { error } = await supabaseAdmin.from('users').update({ totp_enabled: true, recovery_codes: recoveryCodes, updated_at: new Date().toISOString() }).eq('id', session.user.id);
    if (error) return res.status(500).json({ error: 'Failed to enable 2FA.' });
    return res.status(200).json({ success: true, recoveryCodes });
  }
  if (req.method === 'DELETE') {
    const { error } = await supabaseAdmin.from('users').update({ totp_enabled: false, totp_secret: null, recovery_codes: [], updated_at: new Date().toISOString() }).eq('id', session.user.id);
...
function totp(secret, counter) {
  ...
  const buffer = Buffer.alloc(8); buffer.writeBigUInt64BE(BigInt(counter)); const digest = crypto.createHmac('sha1', key).update(buffer).digest(); const offset = digest[digest.length - 1] & 15;

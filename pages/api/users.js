...
export default async function handler(req, res) {
  ...
  if (req.method === 'GET') {
    const { data, error } = await supabaseAdmin
      .from('users')
      .select('id, email, display_name, active, role, availability, created_at')
      .order('email');
  ...
  if (req.method === 'POST') {
    ...
    if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email) || password.length < 12 || !displayName)
      return res
        .status(400)
        .json({ error: 'Email, display name, and a password of at least 12 characters are required.' });
    const { data, error } = await supabaseAdmin
      .from('users')
      .upsert(
        {
  ...
  if (req.method === 'PATCH') {
    ...
    if (req.body?.availability === 'available' || req.body?.availability === 'away')

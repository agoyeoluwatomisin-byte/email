...
export default async function handler(req, res) {
  ...
  const { error } = await supabaseAdmin.from('password_reset_tokens').insert({ user_id: user.id, token_hash: hash(rawToken), expires_at: new Date(Date.now() + 60 * 60 * 1000).toISOString() });
  ...
  if (process.env.RESEND_API_KEY && process.env.RESEND_FROM_ADDRESS) await new Resend(process.env.RESEND_API_KEY).emails.send({ from: process.env.RESEND_FROM_ADDRESS, to: [user.email], subject: 'Reset your email portal password', text: `Reset your password: ${link}` });

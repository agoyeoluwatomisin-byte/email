...
export default async function handler(req, res) {
  ...
  if (!session) return;
  if (req.method === 'GET') {
    const { data, error } = await supabaseAdmin.from('saved_views').select('*').eq('user_id', session.user.id).order('name').limit(100);
    if (error) return res.status(500).json({ error: 'Failed to load saved views.' });
    return res.status(200).json({ views: data || [] });
  }
  if (req.method === 'POST') {
    const name = String(req.body?.name || '').trim().slice(0, 100);
    if (!name || typeof req.body?.filters !== 'object') return res.status(400).json({ error: 'Name and filters are required.' });
    const { data, error } = await supabaseAdmin.from('saved_views').insert({ user_id: session.user.id, name, filters: req.body.filters }).select().single();

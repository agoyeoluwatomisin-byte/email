...
export default async function handler(req, res) {
  ...
  if (req.method === 'GET') {
    const { data, error } = await supabaseAdmin.from('user_signatures').select('signature_html, signature_text, updated_at').eq('user_id', session.user.id).maybeSingle();
  ...
  const { data, error } = await supabaseAdmin.from('user_signatures').upsert({

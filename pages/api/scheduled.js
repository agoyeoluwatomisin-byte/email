...
export default async function handler(req, res) {
  ...
  if (req.method === 'GET') {
    const { data, error } = await supabaseAdmin.from('scheduled_emails').select('id, scheduled_at, status, payload, created_at').eq('user_id', session.user.id).order('scheduled_at', { ascending: true }).limit(100);
  ...
  if (req.method === 'DELETE') {
    ...
    const { error } = await supabaseAdmin.from('scheduled_emails').update({ status: 'cancelled' }).eq('id', id).eq('user_id', session.user.id).eq('status', 'pending');
  ...
  if (Number.isNaN(scheduledAt.getTime()) || scheduledAt.getTime() < Date.now() + 60_000 || scheduledAt.getTime() > Date.now() + 365 * 24 * 60 * 60 * 1000) {

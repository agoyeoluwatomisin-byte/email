...
export default async function handler(req, res) {
  ...
  let { data: emails, error } = await supabaseAdmin
    .from('emails')
    .select('direction, label, received_at, from_address, thread_id, first_response_at, resolved_at')
  ...
  if (error && isMissingAnalyticsColumn(error)) {
    const fallback = await supabaseAdmin
      .from('emails')
      .select('direction, label, received_at, from_address, thread_id')
      .gte('received_at', from.toISOString())
      .lte('received_at', to.toISOString())
      .limit(10000);
  ...
  for (const email of emails || []) {
    ...
    if (email.first_response_at)
      firstResponseTimes.push((new Date(email.first_response_at) - new Date(email.received_at)) / 60000);
    if (email.resolved_at) resolutionTimes.push((new Date(email.resolved_at) - new Date(email.received_at)) / 60000);
  }
  const { data: resolvedThreads } = await supabaseAdmin
    .from('threads')
    .select('thread_id, status, claimed_by, updated_at')
    .eq('status', 'closed')
  ...
  return res.status(200).json({
    volume,
    labels,
    agents: Object.entries(agents).map(([email, replies]) => ({
      email,
      replies,
      resolved: (resolvedThreads || []).filter((thread) => thread.claimed_by === email).length,
    })),
    ...
    overdueCount: (emails || []).filter(
      (email) =>
        !email.first_response_at &&
        email.direction === 'inbound' &&
        Date.now() - new Date(email.received_at).getTime() > 4 * 60 * 60 * 1000,
    ).length,

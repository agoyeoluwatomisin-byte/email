...
export default async function handler(req, res) {
  ...
  if (req.method === 'POST' || req.method === 'PATCH') {
    ...
    const name = String(body.name || '')
      .trim()
      .slice(0, 160);
    if (!name || typeof body.conditions !== 'object' || typeof body.actions !== 'object')
    ...
    const values = {
      ...
      order_index: Math.max(0, Math.min(10000, Number(body.order_index) || 0)),
      updated_at: new Date().toISOString(),
    };
    const query =
      req.method === 'POST'
        ? supabaseAdmin.from('rules').insert(values).select().single()
        : supabaseAdmin
            .from('rules')
            .update(values)

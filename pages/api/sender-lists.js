...
export default async function handler(req, res) {
  ...
  if (req.method === 'POST') {
    ...
    if (!value || !listType || (!value.includes('@') && value.includes(' ')))
      return res.status(400).json({ error: 'A valid address or domain and list type are required.' });
    const { data, error } = await supabaseAdmin
      .from('sender_lists')
      .upsert({ value, list_type: listType }, { onConflict: 'value' })
      .select()
      .single();
